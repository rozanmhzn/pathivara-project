...
//     // router.push(`/blogs/${encodeURIComponent(index)}`);
//   }
...
const LatestNews = () => {
  ...
  const onClick = (title) => {
    ...
    router.push(`/blogs/${encodedTitle}`);
  };
  ...
  return (
    <div className="mt-5">
      ...
      <section className="slider-container mx-10 mt-5">
        <Slider {...settings}>
          {newsData?.map((data, index) => {
            return (
              <div key={index} onClick={() => onClick(data?.title)}>
                <LatestNewsCard news={data} />
              </div>
            );
          })}
        </Slider>
